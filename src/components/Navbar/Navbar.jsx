...
const Navbar = () => {
  ...
  return (
    <nav className="sticky top-0 z-50 shadow-lg">
      ...
      {/* TOP BAR */}
      <div className="bg-luxury text-primaryLight text-xs py-1.5 px-2 sm:px-4">
      ...
      {/* MAIN NAVBAR */}
      <div className="bg-gradient-to-r from-primary via-primaryDark to-luxury px-3 sm:px-4 py-2.5 sm:py-3">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center gap-2 sm:gap-4">

            {/* MOBILE MENU BUTTON */}
            <button
              className="text-white md:hidden order-1"
              onClick={() => setMobileMenu(!mobileMenu)}>
              {mobileMenu ? <X size={22} /> : <Menu size={22} />}
            </button>

            {/* LOGO */}
            <Link to="/" className="flex flex-col min-w-fit order-2 md:order-none">
              <span className="font-elegant text-lg sm:text-2xl font-bold text-white tracking-wide">
                Fashion<span className="text-gold">Store</span>
              </span>
              <span className="hidden sm:block text-primaryLight text-xs tracking-widest uppercase">
                ✦ Luxury Fashion ✦
              </span>
            </Link>
            ...
            <div className="flex items-center gap-3 sm:gap-5 text-white ml-auto order-3 md:order-none">
              ...
              {isLoggedIn ? (
                <div className="relative group cursor-pointer flex flex-col items-center text-xs text-white">
                  <div className="bg-white bg-opacity-20 rounded-full p-1.5 sm:p-2 hover:bg-gold transition">
                    <User size={17} />
                  </div>
                  <span className="hidden sm:flex mt-1 items-center gap-0.5">
                    Hi, {currentUser?.name?.split(" ")[0]} <ChevronDown size={10} />
                  </span>

                  {/* DROPDOWN */}
                  <div className="absolute right-0 top-9 sm:top-11 bg-white text-gray-800 rounded-xl shadow-xl border border-gray-100 py-2 w-44 hidden group-hover:block z-50 overflow-hidden">
                ...
                <Link to="/login"
                  ...
                  <div className="bg-white bg-opacity-20 rounded-full p-1.5 sm:p-2 group-hover:bg-gold transition">
                    <User size={17} />
                  </div>
                  <span className="hidden sm:block mt-1">Login</span>
                </Link>
              )}
              ...
              <Link to="/wishlist"
                ...
                <span className="hidden sm:block mt-1">Wishlist</span>
                {wishlistCount > 0 && (
                  <span className="absolute -top-1 -right-1 bg-gold text-black text-[10px] sm:text-xs rounded-full w-4 h-4 flex items-center justify-center font-bold animate-bounce">
                    {wishlistCount}
                  </span>
                )}
              </Link>
              ...
              <Link to="/cart"
                ...
                <span className="hidden sm:block mt-1">Cart</span>
                {cartCount > 0 && (
                  <span className="absolute -top-1 -right-1 bg-gold text-black text-[10px] sm:text-xs rounded-full w-4 h-4 flex items-center justify-center font-bold">
                    {cartCount}
                  </span>
                )}
              </Link>
