...
const BottomNav = () => {
  ...
  return (
    <div
      className={`fixed bottom-0 left-0 right-0 bg-white/90 backdrop-blur-lg border-t border-gray-100 shadow-[0_-4px_20px_rgba(0,0,0,0.08)] flex md:hidden z-50 transition-transform duration-300 ${
      ...
      <Link to="/cart" className={navItemClass(isActive("/cart"))}>
        ...
        <div className="relative">
          ...
          {cartCount > 0 && (
            <span className="absolute -top-1.5 -right-2 bg-gold text-black text-[9px] rounded-full w-3.5 h-3.5 flex items-center justify-center font-bold animate-pulse">
              {cartCount}
            </span>
          )}
        </div>
